...
<div id="has-many-{{$column}}" class="nav-tabs-custom has-many-{{$column}}">
            ...
            @foreach($forms as $pk => $form)
                <li class="@if ($form == reset($forms)) active @endif ">
                    <a href="#{{ str_replace('.', '-', $relationName) . '_' . $pk }}" data-toggle="tab">
                        {{ config('translatable.native_locale.' . $pk, $pk) }} <i
                ...
                @if(is_array($locale))
                    @foreach($locale as $national)
                        ...
                        <li class="@if ($locale == config('app.locale')) active @endif">
                            <a href="#{{ str_replace('.', '-', $relationName) . '_' . $language_index }}"
                               data-toggle="tab">
    ...
    <div class="tab-content has-many-{{$column}}-forms">
        @if(!empty($forms))
            ...
            @endforeach
        @else
            @foreach(config('laravellocalization.localesOrder') as $key => $locale)
                @if(is_array($locale))
                    @foreach($locale as $national)
                        @php
                            $language_index = $key . '-' . $national;
                        @endphp
                        <div
                                class="tab-pane fields-group has-many-{{$column}}-form @if ($language_index == config('app.locale')) active @endif"
                                id="{{ str_replace('.', '-', $relationName) . '_' . $language_index }}">
                            @foreach($template_fields as $field)
                                ...
                                {!! $field->render() !!}
                            @endforeach
                            <input type="hidden" name="{{$relationName}}[{{ $language_index }}][loc]"
                ...
                @else
                    <div
                            class="tab-pane fields-group has-many-{{$column}}-form @if ($locale == config('laravellocalization.localesOrder')[0]) active @endif"
                            id="{{ str_replace('.', '-', $relationName) . '_' . config('laravellocalization.localesOrder')[$key] }}">
                        @foreach($template_fields as $field)
                            @php
                                $field->setElementName($column.'['. config('laravellocalization.localesOrder')[$key] .'][' .$field->column() .']');
                                $field->attribute('name',$column.'['. config('laravellocalization.localesOrder')[$key] .'][' .$field->column() .']');
                                $field->attribute('title',$field->column());
                                if($field->column() == 'locale'){
